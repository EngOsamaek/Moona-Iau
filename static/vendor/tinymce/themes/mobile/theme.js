    ...
    var wrapApi = function (bName, apiFunction, apiName) {
      var f = function (component) {
        ...
        return component.config({ name: constant(bName) }).fold(function () {
          throw new Error('We could not find any behaviour configuration for: ' + bName + '. Using REST_API: ' + apiName);
        }, function (info) {
    ...
    var build = function (spec) {
      ...
      var config = function (behaviour) {
        if (behaviour === apiConfig()) {
          ...
          throw new Error('Invalid input: only REST_API constant is allowed');
        }
        var b = bData;
